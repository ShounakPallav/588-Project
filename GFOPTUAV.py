...
def runOptimization(initK, baseDir=None, alg="NM"):
    ...
    label = buildLabel(k0)
    outDir = baseDir if baseDir else f"slicesUAVGF_{label}"
    ...
    bounds_lb = np.full(len(k0), -10.0)
    bounds_ub = np.full(len(k0), 10.0)
    ...
    def evalCostSilent(k):
        *_, Ts_h, M_h, SSe_h, SSe_V, vmax, ok = once(k, count=False)
        feas = ok and np.isfinite(Ts_h) and np.isfinite(M_h) and np.isfinite(SSe_h) and np.isfinite(SSe_V) and np.isfinite(vmax)
        viol = feasibilityResidual(M_h, SSe_h, SSe_V, vmax)
        for ki, lb in zip(k, bounds_lb):
            viol = max(viol, lb - ki)
        for ki, ub in zip(k, bounds_ub):
            viol = max(viol, ki - ub)
        if not feas:
            return np.inf
        return Ts_h + 1e3 * viol * viol
    ...
    def penalized_cost(k, tag="NMeval "):
        ...
        cost = Ts_h if (feas and np.isfinite(Ts_h)) else 1e6
        total = cost + penalty
        print(f'{tag}{evalCount["n"]:04d}: '
              f'Kp_h {k[0]:.6g} Ki_h {k[1]:.6g} Kp_th {k[2]:.6g} Ki_th {k[3]:.6g} '
              f'Kd_th {k[4]:.6g} Kp_V {k[5]:.6g} Ki_V {k[6]:.6g}  '
              f'Ts_h {Ts_h:.6g}  M_h {M_h:.6g}  SSe_h {SSe_h:.3e}  '
              f'SSe_V {SSe_V:.3e}  Vmax {vmax:.3g}  viol {viol:.3g}  cost {total:.6g}')
        return total
    ...
    if alg != "NM":
        raise ValueError("Unknown algorithm (supported: 'NM')")

    print('Python: start solve (Nelder-Mead)', flush=True)
    nm_iter = {'i': 0}

    def callback(xk):
        nm_iter['i'] += 1
        data = once(xk, count=False)
        (_, _, _, _, _,
         Ts_h, M_h, SSe_h, SSe_V, vmax, ok) = data
        feas = (M_h <= 0.10 and abs(SSe_h) <= 1e-3 and
                vmax <= 22.0 and abs(SSe_V) <= 1e-3)
        elapsed = time.time() - startTime
        gn = gradNormEst(xk, f0=Ts_h)
        feasRes = feasibilityResidual(M_h, SSe_h, SSe_V, vmax)
        iterIdx = nm_iter['i']
        print(f'NMiter {iterIdx:03d}: k={xk}  Ts_h {Ts_h:.5g}  M_h {M_h:.5g}  '
        ...
        logRow(iterIdx, xk, Ts_h, M_h, SSe_h,
               SSe_V, vmax, feas, feasRes, elapsed, gn)
        savePlot(xk, idx=iterIdx, data=data)
