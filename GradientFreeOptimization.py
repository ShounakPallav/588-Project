...
def runOptimization(initK, baseDir=None):
    ...
    with open(logPath, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['NMiter', 'kp', 'ki', 'kd', 'Ts', 'M', 'SSe', 'feas', 'feasResidual', 'Time(s)', 'gradnorm', 'funCalls'])
    ...
    def penalized_cost(k):
        ...
        total = cost + penalty
        print(f'NMeval {evalCount["n"]:04d}: kp {k[0]:.16g} ki {k[1]:.16g} kd {k[2]:.16g}  Ts {ts:.6g}  M {m:.6g}  SSe {sse:.6g}  viol {viol:.3g}  cost {total:.6g}')
        return total
    ...
    # final log/plot if not already captured in callback
...
if __name__ == "__main__":
    # runOptimization(np.array([0.5, 0.5, 0.5])) # ran already
    # runOptimization(np.array([0.5, 0.5, -0.5])) # ran already
    runOptimization(np.array([-0.5, 0.5, 0.5])) # ran already
